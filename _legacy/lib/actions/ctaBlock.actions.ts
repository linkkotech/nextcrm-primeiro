...
import { prisma, getAuthSession } from '@/lib';
...
export async function saveCTABlock(
  ...
  try {
    ...
    const currentContent = template.content as UnknownData;
    const dynamicBlocks = (currentContent.dynamicBlocks as UnknownData[]) || [];

    // Find and update existing block, or add new one
    const existingBlockIndex = dynamicBlocks.findIndex(
      (block: UnknownData) => block.id === blockId
    );
...
export async function deleteCTABlock(
  ...
  try {
    ...
    const currentContent = template.content as UnknownData;
    const dynamicBlocks = ((currentContent.dynamicBlocks as UnknownData[]) || []).filter(
      (block: UnknownData) => block.id !== blockId
    );
