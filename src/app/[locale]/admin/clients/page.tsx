 ...
 */
function serializeClients(clients: UnknownData[]): SerializedClient[] {
  return clients.map((client: UnknownData) => ({
    id: client.id,
    name: client.name,
    email: client.email,
    image: client.image,
    ...
    workspaceMemberships: Array.isArray(client.workspaceMemberships) ? (client.workspaceMemberships as any[]).map((membership: any) => ({
      workspace: {
        ...
        subscription: {
          ...
          plan: {
            ...
            price: membership.workspace?.subscription?.plan?.price ? Number(membership.workspace.subscription.plan.price) : 0,
          },
        },
      },
      workspaceRole: {
...
export default async function ClientsPage({ 
  ...
  return <ClientsPageClient clients={clients as unknown as Client[]} availablePlans={availablePlans as unknown as Plan[]} initialSearch={searchTerm} />;
}
